...
import com.acme.fromzeroapi.message.application.internal.outboundservices.ExternalIamMessageService;
...
public class MessageController {
    ...
    private final ExternalIamMessageService externalIamMessageService;
    private final MessageQueryService messageQueryService;

    public MessageController(
            MessageCommandService messageCommandService,
            ExternalIamMessageService externalIamMessageService,
            MessageQueryService messageQueryService) {
        this.messageCommandService = messageCommandService;
        this.externalIamMessageService = externalIamMessageService;
    ...
    public ResponseEntity<MessageResource>createMessage(@RequestBody CreateMessageResource resource) {
        ...
        var recipientUser = this.externalIamMessageService.getUserByIdExternalService(resource.recipientId());
        if (recipientUser == null) {
            return ResponseEntity.badRequest().build();
        }

        var senderUser = this.externalIamMessageService.getUserByIdExternalService(resource.senderId());
        if (senderUser == null) {
            return ResponseEntity.badRequest().build();
        }
    ...
    public ResponseEntity<List<MessageResource>> getMessages(@PathVariable Long recipientId) {
        var user = this.externalIamMessageService.getUserByIdExternalService(recipientId);
        if (user == null) {
            return ResponseEntity.badRequest().build();
        }
